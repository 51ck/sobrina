...
- [ ] **T10.1** Migration: `chats` (stable core chat id) + chat settings columns/table (Reminder time, Deadline time, timezone, N; N default **3**)
- [ ] **T10.2** Migration: Checklist membership (chat + member; join/leave timestamps as needed)
...
- [ ] **T11.1** `getOrCreateChat(chatId)` + `getSettings(chatId)`
- [ ] **T11.2** `updateSettings(chatId, patch)` — Reminder time, Deadline time, timezone, N; reject invalid values
- [ ] **T11.3** Defaults: N = **3** when unset; document time/TZ representation chosen in core
